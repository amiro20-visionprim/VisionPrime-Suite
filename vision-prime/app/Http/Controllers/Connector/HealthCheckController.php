...
class HealthCheckController
{
    public function __invoke(Request $request, VerifyConnectorSignature $verify): JsonResponse
    {
        ...
        $health = $request->validate(['plugin_version' => ['required', 'string', 'max:50'], 'wordpress_version' => ['required', 'string', 'max:50'], 'php_version' => ['required', 'string', 'max:50'], 'rest_api' => ['required', 'boolean']]);
        \DB::table('site_connections')->where('id', $connection->id)->update(['plugin_version' => $health['plugin_version'], 'health' => json_encode($health), 'last_seen_at' => now(), 'updated_at' => now()]);
        \DB::table('connector_events')->insert(['site_id' => $connection->site_id, 'type' => 'health.checked', 'payload_redacted' => json_encode($health), 'occurred_at' => now()]);
