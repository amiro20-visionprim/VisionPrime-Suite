...
class ScoreRevenueOpportunities
{
    public function handle(Site $site): int
    {
        ...
        foreach ($insights as $i) {
            ...
            // Too little data to be a reliable signal.
            if ($im < 20) {
                continue;
            }
            ...
            $explanation = sprintf(
                'عبارت «%s» با %d نمایش و جایگاه %s پتانسیل درآمدی بالایی دارد؛ با بهبود عنوان متا و محتوا می‌توان کلیک‌ها و تبدیل را افزایش داد.',
                $i->query_normalized,
                (int) $im,
                number_format($position, 1)
            );
            \DB::table('opportunities')->updateOrInsert(['site_id' => $site->id, 'keyword_insight_id' => $i->id, 'type' => 'revenue_opportunity'], ['url_profile_id' => $i->mapped_url_profile_id, 'score' => $score, 'confidence' => .65, 'status' => 'open', 'explanation' => $explanation, 'updated_at' => now(), 'created_at' => now()]);
