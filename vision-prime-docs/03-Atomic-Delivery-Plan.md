...
### آپدیت #۵۳ — ورود آسان (OTP) + ثبتنام با شماره وریفای + دارکمود پیشفرض (۲۰۲۶-۰۸-۱۸)

- [x] **ورود با کد یکبارمصرف (OTP):** تب «کد یکبارمصرف» در صفحهٔ ورود — شماره تماس → کد ۶ رقمی (کاوهنگار) → ورود بدون رمز؛ sandbox (بدون کلید) کد را نمایش میدهد
- [x] **ثبتنام با شماره تماس وریفای:** فیلد شماره + دکمهٔ «دریافت کد» + وریفای قبل از ساخت حساب؛ خط وریفیشده (phone_verified_at) ثبت میشود
- [x] **چکباکس پذیرش قوانین:** «ثبتنام به منزلهٔ پذیرفتن قوانین و سیاستهای Vision Prime SUITE» — اجباری
- [x] **کاهش حداقل رمز به ۸ کاراکتر** (RegisterRequest + LoginRequest + hint UI)
- [x] **دکمهٔ نمایش/پنهان پسورد** با آیکون Eye/EyeOff مدرن در ورود و ثبتنام
- [x] **دارکمود پیشفرض** (theme.ts + anti-FOUC بلید) + توگل در صفحات ورود (AuthLayout) و مارکتینگ (MarketingHeader)
- [x] **زیرساخت:** OtpService (کد هششده، انقضا ۵ دقیقه، محدودیت ۵ تلاش) + ۲ کنترلر + ۳ روت + migration phone
- [x] **تست:** ۳۶۸ تست / ۱۹۳۰ assert سبز (۶ تست OTP جدید + RegisterTest بهروز) + pint + typecheck + build
- [x] **استقرار:** migration روی سرور + بیلد جدید (`app-DX6UKpiN.js`) + تأیید OTP endpoint (کد sandbox برگشت)
